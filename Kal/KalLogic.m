...
- (id)initForDate:(NSDate *)date
{
  if ((self = [super init])) {
    ...
    dayMonthAndYearFormatter = [[NSDateFormatter alloc] init];
    [dayMonthAndYearFormatter setDateFormat:@"d LLLL yyyy"];
...
- (NSString *)selectedDayMonthNameAndYear:(KalDate *)date
{
    return [dayMonthAndYearFormatter stringFromDate:date.NSDate];
}
